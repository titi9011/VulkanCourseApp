...
int VulkanRenderer::init(GLFWwindow* newWindow)
{
	...
	try {
		...
		mvp.projection = glm::perspective(glm::radians(45.f), (float)swapChainExtent.width / (float)swapChainExtent.height, 0.1f, 100.0f);
...
void VulkanRenderer::createDescriptorSetLayout()
{
	...
	mvpLayoutBinding.descriptorCount = 1;									// Number of descriptors for biding
	mvpLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;				// Shader stage to bind to
	mvpLayoutBinding.pImmutableSamplers = nullptr;							// For texture: Can make sampler immuable (data is mutable) by spceifying in layout
...
void VulkanRenderer::createDescriptorSets()
{
	...
	setAllocInfo.descriptorSetCount = static_cast<int32_t>(uniformBuffer.size());		// Number of sets to allocate
